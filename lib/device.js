...
class Device extends EventEmitter {
    ...
    constructor(opts) {
        ...
        this.on('osk_string_changed', packet => {
            if (this.opts.debug) {
                console.log('OSK', packet);
            }
            this._osk = packet;
        });
    }
     ...
     */
    async oskSetText(string, caretIndex=-1) {
        await this.startOsk();
     ...
     * Send an OSK command:
     *  - 'close': Close the OSK
     *  - 'return': Submit the current value
     * Resolves to this object.
     */
    async oskCommand(command) {
        let socket = await this.openSocket();

        socket.sendOskCommand(command.toLowerCase());
        return this;
    }
    ...
    /**
     * Attempt to start on-screen keyboard mode. Resolves
     *  to an object with information about the mode.
     *  In most cases you will want to prefer the other
     *  osk* methods.
     */
    async startOsk() {
        let socket = await this.openSocket();
