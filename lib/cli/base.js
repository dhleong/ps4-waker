...
class CommandOnDevice {
    ...
    async _search(ui, onEach) {
        ...
        const condition = (device, rinfo) => !detectOpts.device
            || rinfo.address === detectOpts.device;

        // if either a device is provided OR there's no timeout,
        //  we just quickly stop on the first found; otherwise,
        //  just keep going
        const detectorFunction = detectOpts.device || detectOpts.timeout === undefined
    ...
    _registerDevice(ui, device, creds) {
        ...
        sock.on('login_result', (packet) => {
            if (packet.result === 0) {
                ui.logResult('Logged into device! Future uses should succeed');
                ui.exitWith(0);
            } else {
