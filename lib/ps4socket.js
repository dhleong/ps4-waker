...
Ps4Socket.prototype.close = function() {
    ...
    // don't wait forever for the logout request...
    const timeout = setTimeout(doDisconnect, 15000);

    // ... but at least wait a bit
    this.once('logout_result', (packet) => {
        // check packet.status
        if (packet.status) {
            this.log('WARN: Logout result:', packet.status);
        }
...
Ps4Socket.prototype.login = function(pinCodeOrConfig, callback) {
    ...
    if (cb) {
        ...
        loginTimeout = setTimeout(() => {
            this.log(`Login timed out; ${config.retries} retries remaining`);
            this.removeListener('login_result', onLoginResult);

            config.retries -= 1;
            if (config.retries > 0) {
                // try again after a short delay
                this.log(`Retry login in ${LOGIN_RETRY_DELAY}`);
...
KnownPackets = {
    ...
    0x6f636370: function(packet) {
        ...
        if (this.config.autoLogin) {
            ...
            this.login(this.config, (result) => {
                this.log('Auto-Login result:', result);
            });
        } else {
