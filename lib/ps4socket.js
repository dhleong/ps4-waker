...
Ps4Socket.prototype.login = function(pinCode, callback) {
    ...
    if (!callback && typeof(pinCode) == 'function') {
        callback = pinCode;
        pinCode = "";
    }
    ...
    if (callback) {
        this.once('login_result', function on_login_result(result) {
...
Ps4Socket.prototype.startTitle = function(titleId) {
...
function newBootRequestPacket(titleId) {
    return new Packet(8 + 16)
        .writeInt(10) // packet type
        .write(titleId, 16);
}

function newBootRequest2Packet(titleId) {
    var i = 0; // first param (no idea what it is)
    var j = 0; // second param (always 0?)
    return new Packet(92 + 16)
        .writeInt(36) // packet type
        .writeInt(i)
        .write("", 12) // no idea; the app does this, too
        .write("", 64) // no idea; the app does this, too
        .writeInt(j)
        .write(titleId, 16)
}
