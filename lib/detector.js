...
const debug = require('debug')('ps4:detector');
 ...
 *          wishes to assign
 */
Detector.prototype.detect = function(detectOptsGiven) {
    const self = this;
    const s = ps4lib.udpSocket();
    this.socket = s;

    let detectOpts = detectOptsGiven;
    ...
    s.bind(detectOpts.bindPort, detectOpts.bindAddress, () => {
        s.setBroadcast(true);

        s.on('message', (msg, rinfo) => {
            const packet = ps4lib.parse(msg);
            debug('received', packet, 'from', rinfo);

            const type = packet.type.toLowerCase();
            self.emit(type, packet, rinfo);
        });
        s.on('error', (err) => {
            self.emit('error', err);
        });
...
/**
 * Find devices for whom filterFn returns `true`, continuing until
 * either a timeout hits, or callback returns true
 *
 ...
 * @return a Promise that resolves when we finish detecting
 */
Detector.findWhen = function(filterFn, detectOpts, callback) {
    ...
    return new Promise((resolve) => {
        ...
        detector.on('close', onTimeout);

        detector.on('device', (device, rinfo) => {
            ...
            if (filterFn(device, rinfo)) {
                ...
                detector.removeListener('close', onTimeout);

                const result = callback(null, device, rinfo);
                if (result) {
...
Detector.findAny = function(detectOpts, callback) {
    return Detector.findFirst(() => true, detectOpts, callback);
};

Detector.find = function(address, detectOpts, callback) {
    return Detector.findFirst((detected, rinfo) => {
